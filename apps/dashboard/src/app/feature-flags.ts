export const FeatureFlags = {
  ...
  SIGNAL_FORMS_ROSTER: true,  // SignalFormsRosterComponent — split-lease roster form (Sprint 058), the dynamic-array PoC in the Signal Forms migration line. Proves the surface the scalar PoCs never touched: a repeating array of co-tenant sub-objects ({name,email,sharePct}) via applyEach(path.tenants, tenantSchema) for per-item validation, an array-level minLength (≥1 tenant), a cross-item/tree rule via validate(path.tenants, …) (shares must sum to 100%), and live add/remove mutating the model signal. De-risks the bespoke→official migration for collection fields. Additive, replaces nothing.
  SIGNAL_FORMS_ASYNC: true,  // SignalFormsAsyncComponent — async-validation PoC (Sprint 059), the last unproven surface in the Signal Forms migration line. A listing-reference claim field checked for uniqueness against a (mock) backend via validateAsync(path, {params, factory: resource(), onSuccess, onError}): sync validators gate the async check (a too-short/invalid ref never hits the backend), field().pending() drives a live spinner + disabled submit, and a debounce delays the resource until typing stops. Resolver + debounce are overridable for deterministic tests. De-risks the bespoke→official migration for server-side/uniqueness checks. Additive, replaces nothing.
  AUTH_REGISTER_SIGNAL_FORM: true,  // AuthRegisterSignalFormComponent — first migration of a REAL product form (auth-register) off the bespoke createSignalForm util onto Angular 22's official @angular/forms/signals form() (Sprint 060). Faithful side-by-side twin of AuthRegisterComponent: identical markup/styles/outputs/AuthService wiring, only the form engine differs. Confirm-password is now a first-class model field validated cross-field via validate(path.confirmPassword, ({valueOf}) => …) against password (the bespoke kept it as a standalone signal). A parity spec asserts both twins hand register() a byte-identical RegisterData. Bespoke stays the shipped default; this replaces nothing. Additive.
  TENANT_APPLICATION_SIGNAL_FORM: true,  // TenantApplicationSignalFormComponent — second migration of a REAL product form (the multi-step tenant rental application) off the bespoke createSignalForm util onto Angular 22's official @angular/forms/signals form() (Sprint 061). Faithful side-by-side twin of TenantApplicationFormComponent: identical 5-step flow, markup, styles, outputs and TenantApplicationService wiring, only the form engine differs. Proves the migration on surfaces the auth twin (#060) didn't touch: the NIF pattern(/^\d{9}$/) validator, native number parse on monthlyIncome/numOccupants ([formField] on type=number, replacing manual Number() coercion), and imperative field writes via f.field().value.set() for the segmented employment/pets choosers. References stay a plain signal in both twins (never lived in the bespoke form). A parity spec asserts both twins hand TenantApplicationService.submit() a byte-identical payload. Bespoke stays the shipped default; this replaces nothing. Additive.
  AUTH_LOGIN_SIGNAL_FORM: true,  // AuthLoginSignalFormComponent — third migration of a REAL product form (auth-login) off the bespoke createSignalForm util onto Angular 22's official @angular/forms/signals form() (Sprint 062), completing the auth pair after auth-register (#060). Faithful side-by-side twin of AuthLoginComponent: identical markup/styles/outputs (loginSuccess/registerRequested/forgotPassword)/AuthService wiring, only the form engine differs. Validators mirror the bespoke twin byte-for-byte (email required+valid, password required + minLength 6); rememberMe stays a standalone UI signal (no text validators). A parity spec asserts both twins hand login() a byte-identical LoginCredentials. Bespoke stays the shipped default; this replaces nothing. Additive.
  MAINTENANCE_REQUEST_SIGNAL_FORM: true,  // MaintenanceRequestSignalFormComponent — fourth migration of a REAL product form (tenant maintenance-request) off the bespoke createSignalForm util onto Angular 22's official @angular/forms/signals form() (Sprint 063). Faithful side-by-side twin of MaintenanceRequestFormComponent: identical markup/styles/inputs/submitted output and MaintenanceRequestService wiring, only the form engine differs. Proves two surfaces the auth/tenant twins only partially touched, all via [formField]: <select>/enum for category+priority ('' → required rejects the placeholder) and a <textarea> for description with a maxLength(1000) ceiling, plus minLength/maxLength on both title and description (messages mirror the bespoke defaults byte-for-byte). A parity spec asserts both twins hand create() a byte-identical CreateMaintenanceRequestPayload. Bespoke stays the shipped default; this replaces nothing. Additive.
  LEASE_AGREEMENT_SIGNAL_FORM: true,  // LeaseAgreementSignalFormComponent — fifth migration of a REAL product form (landlord lease-agreement create) off the bespoke createSignalForm util onto Angular 22's official @angular/forms/signals form() (Sprint 064). Faithful side-by-side twin of LeaseAgreementFormComponent: identical markup/styles/inputs/submitted+cancelled outputs and LeaseAgreementService wiring, only the form engine differs. Proves surfaces the earlier twins only partially touched, all via [formField]: a segmented leaseType chooser written imperatively via f.leaseType().value.set(), two <input type="date"> fields, two <input type="number"> fields (monthlyRent/depositAmount as number|null, native parse — bespoke coerced strings on submit), a minLength(50) terms <textarea>, and an optional validator-free notes field. Messages mirror the bespoke validators byte-for-byte. A parity spec asserts both twins hand create() a byte-identical CreateLeasePayload. Bespoke stays the shipped default; this replaces nothing. Additive.
  PROPERTY_BOOKING_SIGNAL_FORM: true,  // PropertyBookingSignalFormComponent — sixth migration of a REAL product form (property booking/inquiry panel) off the bespoke createSignalForm util onto Angular 22's official @angular/forms/signals form() (Sprint 065). Faithful side-by-side twin of PropertyBookingComponent: identical markup/styles/property input/bookingSubmitted+closed outputs and emitted BookingSubmitEvent, only the form engine differs. Proves the two-mode CONDITIONAL form the earlier twins sidestepped: a visit/inquiry tab swaps which optional fields render and calls f().reset() (official FieldState reset, clearing values + touched) on switch, mirroring the bespoke form.reset(). All controls via [formField]: text/email inputs, a <select> time-slot, <input type=date> fields, a maxLength(500) <textarea>. The bespoke [min]=todayISO native picker hint is dropped (native min on a [formField] node = NG8022; it was only a UI hint, never a validator — submitted payload unchanged). A parity spec asserts both twins emit a byte-identical BookingSubmitEvent.form. Bespoke stays the shipped default; this replaces nothing. Additive.
  ...
  PAYMENT_SHOWCASE: true,  // NG-06 — página dedicada `/payment-showcase` (proof surface do marco 9b) que exercita o UM `<iu-payment>` em todos os intents (checkout|deposit|refund) e nos 8 estados do ciclo (idle→validating→ready→processing→success, error+retry, terminal cancelled/expired). SEGURA de mostrar independente de PAYMENT_V2: o gateway raiz é um stub em modo teste (sem chaves, sem rede, sem dinheiro). Gate separado do PAYMENT_V2 (que controla a adoção nas superfícies de produto) para o Israel poder ver o marco sem ligar o pagamento em produção. Additivo: rota lazy nova, não toca em página existente.
} as const;
