/**
 * @fileoverview BookingCheckoutSignalFormComponent — Signal Forms migration #7
 *
 * Faithful twin of {@link BookingCheckoutComponent} (Sprint 028) whose ONLY
 * difference is the engine behind the **Payment** step's form: the bespoke
 * `createSignalForm({ … })` util is replaced by the official
 * `form(model, schema)` API from `@angular/forms/signals`. Every other surface —
 * the review/payment/confirmation state machine, the summary breakdown, the
 * method selector, the confirmation screen, styles, inputs and outputs — mirrors
 * the original byte-for-byte.
 *
 * This closes the Signal Forms migration line: `booking-checkout` was the last
 * real product form still on the bespoke util (after auth-register, auth-login,
 * tenant-application, maintenance-request, lease-agreement, property-booking).
 *
 * Migration map (bespoke `createSignalForm` → official `form()`):
 * | bespoke                                              | official                                          |
 * | --------------------------------------------------- | ------------------------------------------------- |
 * | `createSignalForm({ cardHolder: { validators } })`  | `form(model, (p) => { required(p.cardHolder) … })`|
 * | `paymentForm.fields['x'].value()`                   | `f.x().value()`                                   |
 * | `…fields['x'].setValue(v)`                          | `f.x().value.set(v)`                              |
 * | `…fields['x'].touch()`                              | `f.x().markAsTouched()`                           |
 * | `…fields['x'].showError()` / `.firstError()`        | `showError(f.x)` / `firstError(f.x)` helpers      |
 *
 * The card-number/expiry inputs keep explicit `[value]`/`(input)` bindings
 * (NOT `[formField]`) because they run `formatCardNumber`/`formatExpiry` on
 * every keystroke — the two-way `[formField]` directive can't transform input.
 * Keeping explicit bindings also side-steps the NG8022 gotcha: the native
 * `maxlength` attributes stay plain HTML attributes, never colliding with a
 * `[formField]` on the same node (see memory `ng-m3_formfield_forbids_native_constraints`).
 *
 * Feature flag: BOOKING_CHECKOUT_SIGNAL_FORM (bespoke stays default; additive).
 *
 * @example
 * ```html
 * <iu-booking-checkout-signal-form
...
import {
  ...
  CheckoutStep,
} from '../booking-checkout/booking-checkout.component';

// ─── Payment form model ─────────────────────────────────────────────────────

/** Flat model wrapped by `form()`; empty-string sentinels for each control. */
...
/**
 * BookingCheckoutSignalFormComponent — orchestrates the full booking checkout
 * flow, with the Payment step's form powered by the official `form()` API.
 *
 * Steps:
 * - **review**: Displays property summary, date range, price breakdown, and a "Proceed to Payment" CTA.
 * - **payment**: Collects payment method + credentials via the official Signal Forms `form()`. Validates inline.
 ...
 */
@Component({
  selector: 'iu-booking-checkout-signal-form',
  standalone: true,
    ...
    <div class="iu-checkout" role="dialog" aria-modal="true" aria-label="Checkout">
      ...
      <div class="iu-checkout__panel" role="document">
        ...
        <div class="iu-checkout__header">
          ...
          <h2 class="iu-checkout__title">
            {{ stepTitle() }}
            <span class="iu-checkout__badge">form()</span>
          </h2>
          @if (step() !== 'confirmation') {
              ...
              <div class="iu-checkout__card-form">
                <div class="iu-checkout__field">
                  ...
                  <input id="cardHolder"
                         ...
                         [class.iu-checkout__input--error]="showError(f.cardHolder)"
                         [value]="f.cardHolder().value()"
                         (input)="f.cardHolder().value.set($any($event.target).value)"
                         (blur)="f.cardHolder().markAsTouched()"
                         placeholder="NOME SOBRENOME"
                         autocomplete="cc-name" />
                  @if (showError(f.cardHolder)) {
                    <span class="iu-checkout__error">{{ firstError(f.cardHolder) }}</span>
                  }
                </div>
                ...
                <div class="iu-checkout__field">
                  ...
                  <input id="cardNumber"
                         class="iu-checkout__input"
                         [class.iu-checkout__input--error]="showError(f.cardNumber)"
                         [value]="f.cardNumber().value()"
                         (input)="f.cardNumber().value.set(formatCardNumber($any($event.target).value))"
                         (blur)="f.cardNumber().markAsTouched()"
                         placeholder="0000 0000 0000 0000"
                         maxlength="19"
                         autocomplete="cc-number" />
                  @if (showError(f.cardNumber)) {
                    <span class="iu-checkout__error">{{ firstError(f.cardNumber) }}</span>
                  }
                </div>
                ...
                <div class="iu-checkout__field-row">
                  <div class="iu-checkout__field">
                    ...
                    <input id="cardExpiry"
                           class="iu-checkout__input"
                           [class.iu-checkout__input--error]="showError(f.cardExpiry)"
                           [value]="f.cardExpiry().value()"
                           (input)="f.cardExpiry().value.set(formatExpiry($any($event.target).value))"
                           (blur)="f.cardExpiry().markAsTouched()"
                           placeholder="MM/AA"
                           maxlength="5"
                           autocomplete="cc-exp" />
                    @if (showError(f.cardExpiry)) {
                      <span class="iu-checkout__error">{{ firstError(f.cardExpiry) }}</span>
                    }
                  </div>
                  ...
                  <div class="iu-checkout__field">
                    ...
                    <input id="cardCvv"
                           class="iu-checkout__input"
                           [class.iu-checkout__input--error]="showError(f.cardCvv)"
                           [value]="f.cardCvv().value()"
                           (input)="f.cardCvv().value.set($any($event.target).value)"
                           (blur)="f.cardCvv().markAsTouched()"
                           placeholder="123"
                           maxlength="4"
                           autocomplete="cc-csc" />
                    @if (showError(f.cardCvv)) {
                      <span class="iu-checkout__error">{{ firstError(f.cardCvv) }}</span>
                    }
                  </div>
                </div>
              </div>
              ...
              <div class="iu-checkout__mbway-form">
                <div class="iu-checkout__field">
                  ...
                  <div class="iu-checkout__phone-wrap">
                    ...
                    <input id="mbwayPhone"
                           class="iu-checkout__input"
                           [class.iu-checkout__input--error]="showError(f.mbwayPhone)"
                           [value]="f.mbwayPhone().value()"
                           (input)="f.mbwayPhone().value.set($any($event.target).value)"
                           (blur)="f.mbwayPhone().markAsTouched()"
                           placeholder="9XX XXX XXX"
                           maxlength="9"
                           autocomplete="tel-national" />
                  </div>
                  @if (showError(f.mbwayPhone)) {
                    <span class="iu-checkout__error">{{ firstError(f.mbwayPhone) }}</span>
                  }
  ...
  styles: [`
    ...
    /* Signal Forms twin marker — mirrors the badge on the other migrated twins */
    .iu-checkout__badge {
      font: var(--md-sys-typescale-label-small-font, 500 11px/16px 'Google Sans', sans-serif);
      letter-spacing: .5px;
      padding: 2px 8px;
      border-radius: 999px;
...
export class BookingCheckoutSignalFormComponent {
  ...
  /** Empty-string sentinel model; `form()` wraps and mutates it via the bindings. */
  private static readonly EMPTY_MODEL: PaymentFormModel = {
    cardHolder: '',
    cardNumber: '',
    cardExpiry: '',
    cardCvv: '',
  ...
  /** Single source of truth for the payment form. */
  private readonly model = signal<PaymentFormModel>({
    ...BookingCheckoutSignalFormComponent.EMPTY_MODEL,
  });

  /**
   * Field tree with every rule declared inline. Messages mirror the bespoke
   * `createSignalForm` validators byte-for-byte so both twins read identically.
   * `required` catches the empty sentinel; `pattern` skips empties (so `required`
   * owns the empty case) and validates format once a value is present.
   */
  protected readonly f = form(this.model, (p) => {
    required(p.cardHolder, { message: 'Nome obrigatório' });
    minLength(p.cardHolder, 3, { message: 'Mínimo 3 caracteres' });

    required(p.cardNumber, { message: 'Número obrigatório' });
    pattern(p.cardNumber, /^[\d\s]{19}$/, { message: 'Número inválido (16 dígitos)' });

    required(p.cardExpiry, { message: 'Validade obrigatória' });
    pattern(p.cardExpiry, /^\d{2}\/\d{2}$/, { message: 'Formato MM/AA' });

    required(p.cardCvv, { message: 'CVV obrigatório' });
    pattern(p.cardCvv, /^\d{3,4}$/, { message: '3–4 dígitos' });

    required(p.mbwayPhone, { message: 'Telemóvel obrigatório' });
    pattern(p.mbwayPhone, /^9\d{8}$/, { message: 'Número PT inválido' });
  });

  // ── Error-display helpers (mirror the bespoke showError/firstError) ───────

  /** Whether a field should surface its error — touched AND invalid. */
  ...
  onSubmitPayment(): void {
    ...
    if (this.selectedMethod() === 'card') {
      const cardFields = [this.f.cardHolder, this.f.cardNumber, this.f.cardExpiry, this.f.cardCvv];
      cardFields.forEach(fld => fld().markAsTouched());
      if (cardFields.some(fld => fld().invalid())) return;
    } else if (this.selectedMethod() === 'mbway') {
      const phone = this.f.mbwayPhone;
      phone().markAsTouched();
  ...
  private finalisePayment(): void {
    ...
    const paymentData: PaymentFormData = {
      ...
      cardHolder:  method === 'card'  ? this.f.cardHolder().value() : undefined,
      cardNumber:  method === 'card'  ? this.f.cardNumber().value() : undefined,
      cardExpiry:  method === 'card'  ? this.f.cardExpiry().value() : undefined,
      mbwayPhone:  method === 'mbway' ? this.f.mbwayPhone().value() : undefined,
    };
