...
import type { MoveOutCategory } from '../../services/move-out-checklist.service';

interface CategoryMeta {
  readonly id: MoveOutCategory;
...
/**
 * MoveOutChecklistComponent — tenant move-out checklist for Portuguese rentals.
 *
 * Symmetric counterpart to {@link MoveInChecklistComponent} (Sprint 040).
 * Shows progress meter, per-category breakdown, and toggleable task items
 * organised around exit obligations: contract termination, utility
 * cancellation, walk-through, deposit reclaim.
 *
 * Built with M3 design tokens + native primitives only (no `@material/web`
 * components beyond CSS vars).
 *
 * @example
 * <iu-move-out-checklist [showCategoryProgress]="true" />
 */
@Component({
  selector: 'iu-move-out-checklist',
  standalone: true,
  ...
  template: `
    <div class="mc-root">
      ...
      <header class="mc-header">
        <div>
          <h2 class="mc-title">Move-Out Checklist</h2>
          <p class="mc-subtitle">
              ...
              @for (task of group; track task.id) {
                <li class="mc-item" [class.done]="task.done">
                  ...
                  <span class="mc-task-when">
                    @if (task.daysBeforeMoveOut > 0) {
                      D-{{ task.daysBeforeMoveOut }}
                    } @else if (task.daysBeforeMoveOut === 0) {
                      D-0
                    } @else {
                      D+{{ -task.daysBeforeMoveOut }}
                    }
...
export class MoveOutChecklistComponent {
  readonly svc = inject(MoveOutChecklistService);
